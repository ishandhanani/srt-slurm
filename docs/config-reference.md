...
### Container Format (with source for container-pull)

The `containers` field supports two formats:
...
**New format (with source for `srtctl container-pull`)**:
...
| -------- | ------ | -------- | ----------------------------------------------------- |
| `path`   | string | Yes      | Local path to the .sqsh file                          |
| `source` | string | No       | Docker URL for downloading (used by `container-pull`) |

When using `srtctl container-pull --sbatch`, containers with a `source` field will be downloaded via a SLURM batch job using `enroot import`. See [CLI Reference](cli.md#srtctl-container-pull) for details.
...
| `custom`          | User-specified command with placeholders    |
...
Run a user-specified command with placeholder expansion. Allows flexible benchmarking with any tool.

```yaml
benchmark:
  type: "custom"
  command: "uvx aiperf profile --url {nginx_url} --concurrency 128 --artifact-dir {log_dir}/results"
  container_image: "/containers/aiperf.sqsh" # Optional: custom container
```

| Field             | Type              | Required | Default          | Description                          |
| ----------------- | ----------------- | -------- | ---------------- | ------------------------------------ |
| `command`         | FormattableString | Yes      | -                | Command template with placeholders   |
| `container_image` | FormattablePath   | No       | worker container | Custom container to run benchmark in |

**Available Placeholders in `command`:**

| Placeholder       | Description                    | Example                         |
| ----------------- | ------------------------------ | ------------------------------- |
| `{nginx_url}`     | URL of the nginx load balancer | `http://localhost:8000`         |
| `{log_dir}`       | Log directory path             | `/home/user/outputs/12345/logs` |
| `{job_id}`        | SLURM job ID                   | `12345`                         |
| `{run_name}`      | Job name + job ID              | `my-job_12345`                  |
| `{model_path}`    | Resolved model path            | `/model`                        |
| `{head_node_ip}`  | IP address of head node        | `10.0.0.1`                      |
| `{gpus_per_node}` | GPUs per node                  | `8`                             |

**Examples:**

```yaml
# Run AIPerf with custom settings
benchmark:
  type: "custom"
  command: |
    uvx aiperf profile \
      --url {nginx_url} \
      --concurrency 128 \
      --artifact-dir {log_dir}/aiperf-results

# Run a custom benchmark script
benchmark:
  type: "custom"
  command: "bash /scripts/my-benchmark.sh {nginx_url} {log_dir}"
  container_image: "/containers/my-tools.sqsh"

# Use curl to test endpoint
benchmark:
  type: "custom"
  command: "curl -X POST {nginx_url}/v1/completions -H 'Content-Type: application/json' -d '{\"model\": \"test\", \"prompt\": \"Hello\"}'"
```
