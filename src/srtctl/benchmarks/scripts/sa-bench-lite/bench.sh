...
# SA-Bench-Lite: Throughput/latency benchmark (half requests vs sa-bench)
...
MODEL_PATH="/model/"
# Reference the Python files from sa-bench
WORK_DIR="$(dirname "$0")/../sa-bench"

echo "SA-Bench-Lite Config: endpoint=${ENDPOINT}; isl=${ISL}; osl=${OSL}; concurrencies=${CONCURRENCIES}; req_rate=${REQ_RATE}; model=${MODEL_NAME}"
...
# Benchmark
result_dir="/logs/sa-bench-lite_isl_${ISL}_osl_${OSL}"
mkdir -p "$result_dir"

for concurrency in "${CONCURRENCY_LIST[@]}"; do

    # LITE: Half the warmup prompts (concurrency * 1 instead of * 2)
    ...
    # LITE: Half the main prompts (concurrency * 3)
...
echo "SA-Bench-Lite complete. Results in $result_dir"
